...
const orderFaker = async () => {
  ...
  const customer = new Customer(randomUUID(), "Customer 1");
  ...
  const order = new Order(randomUUID(), customer.id, [ordemItem]);
  await orderRepository.create(order);

  return { customer, product, ordemItem, order };
};
...
describe("Order repository test", () => {
  ...
  it("should update order", async () => {
    const orderRepository = new OrderRepository();
    const { product, order, ordemItem } = await orderFaker();
    ...
    expect(orderModel.toJSON()).toStrictEqual(order.toJSON());
  });

  it("should find order", async () => {
    const { order } = await orderFaker();
    const orderRepository = new OrderRepository();
    ...
    expect(SequelizeMapper.toDomain(orderModel).toJSON()).toStrictEqual(
      foundOrder.toJSON()
    );
  });
